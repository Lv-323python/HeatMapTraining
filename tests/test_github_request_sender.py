...
@mock.patch('heat_map_training.request_sender.github_request_sender.requests.get')
@mock.patch(
    'heat_map_training.request_sender.github_request_sender.GithubRequestSender._get_complete_commit_branch_map')
def test_get_commits_success_mock(mock_complete, mock_get):
    mock_get.return_value.status_code = STATUS_CODE_OK
    mock_get.return_value.json.return_value = CMTS_DATA
    mock_complete.return_value = {}
    print("RRR", GithubRequestSender('BoartK', 'test1').get_commits())
    assert GithubRequestSender('BoartK', 'test1').get_commits() == [
        {
            'hash': '8298a9be2ce7b5fd4e3209d6f68bf2c098428144',
            'author': 'BoartK',
            'message': 'about',
            'date': 1528696403,
            'branch': 'unknown'
        ...
        {
            'hash': 'fb21f75ab1dc63aec48a738091abda9f97a73e07',
            'author': 'BoartK',
            'message': 'Frst',
            'date': 1528695133,
            'branch': 'unknown'
...
@mock.patch(
    'heat_map_training.request_sender.github_request_sender.GithubRequestSender._get_complete_commit_branch_map')
def test_get_commits_failed_mock(mock_complete, mock_get):
    mock_get.return_value.status_code = not STATUS_CODE_OK
    mock_get.return_value.json.return_value = CMTS_DATA
    mock_complete.return_value = {}
    assert GithubRequestSender('BoartK', 'unknown').get_commits() is None


@mock.patch('heat_map_training.request_sender.github_request_sender.requests.get')
@mock.patch(
    'heat_map_training.request_sender.github_request_sender.GithubRequestSender._get_complete_commit_branch_map')
def test_get_commit_by_hash_success_mock(mock_complete, mock_get):
    mock_get.return_value.status_code = STATUS_CODE_OK
    mock_get.return_value.json.return_value = CMT_BY_HASH_DATA
    mock_complete.return_value = {}
    assert GithubRequestSender('BoartK', 'test').get_commit_by_hash(
        '8298a9be2ce7b5fd4e3209d6f68bf2c098428144') == {
               'hash': '8298a9be2ce7b5fd4e3209d6f68bf2c098428144',
               'author': 'BoartK',
               'message': 'about',
               'date': 1528696403,
...
@mock.patch(
    'heat_map_training.request_sender.github_request_sender.GithubRequestSender._get_complete_commit_branch_map')
def test_get_commit_by_hash_wrong_attr_fail_mock(mock_complete, mock_get):
    mock_get.return_value.status_code = not STATUS_CODE_OK
    mock_get.return_value.json.return_value = CMT_BY_HASH_DATA
    mock_complete.return_value = {}
    assert GithubRequestSender('BoartK', 'unknown').get_commit_by_hash(
        '8298a9be2ce7b5fd4e3209d6f68bf2c098428144') is None


@mock.patch('heat_map_training.request_sender.github_request_sender.requests.get')
@mock.patch(
    'heat_map_training.request_sender.github_request_sender.GithubRequestSender._get_complete_commit_branch_map')
def test_get_commit_by_hash_wrong_hash_fail_mock(mock_complete, mock_get):
    mock_get.return_value.status_code = not STATUS_CODE_OK
    mock_get.return_value.json.return_value = CMT_BY_HASH_DATA
    mock_complete.return_value = {}
    assert GithubRequestSender('BoartK', 'test').get_commit_by_hash(
