...
class MongoDBRequestSender:
    ...
    def delete_entry(self, body):
        """
        Delete hash of the request to MongoDB database
        :param body:
        :param response:
        :return:
        """
        ...
        key = '-'.join(body.values())
        try:
            print("MongoDBRequestSender.delete_entry: Inserted into MongoDB hash_id:")
            print(self.hash_collection.delete_one({"key": key}))
        except pymongo.errors.PyMongoError as err:
            print("MongoDBRequestSender.delete_entry: Problems with MongoDB")
            print(err)
